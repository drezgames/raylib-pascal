...
begin
  ...
  cam.target.&Set(0.0, 1.5, 0.0);
  cam.up.&Set(0.0, 1.0, 0.0);
  cam.fovy := 45.0;
  cam.&type := CAMERA_PERSPECTIVE;

  dwarf := LoadModel('res/models/dwarf.obj'); // Load OBJ model
  texture := LoadTexture('res/models/dwarf_diffuse.png');
  // Load model texture (diffuse map)
  dwarf.material.maps[MAP_DIFFUSE].texture := texture;
  // Set dwarf model diffuse texture

  position.&Set(0.0, 0.0, 0.0); // Set model position

  shdr := LoadShader('res/shaders/glsl330/base.vs',
    'res/shaders/glsl330/swirl.fs'); // Load postpro shader
  ...
  begin
    ...
    DrawTextureRec(target.texture, TRectangle.Create(0, 0, target.texture.width,
      -target.texture.height), TVector2.Create(0, 0), WHITE);
