...
begin
  ...
  begin
    mouseTail[i].position.&Set(0, 0);
    mouseTail[i].color.&Set(GetRandomValue(0, 255), GetRandomValue(0, 255),
  ...
  begin
    ...
    BeginDrawing;
      ...
      BeginBlendMode(blending);
        ...
        begin
          if mouseTail[i].active then
            DrawTexturePro(smoke, TRectangle.Create(0, 0, smoke.width, smoke.height),
              TRectangle.Create(Round(mouseTail[i].position.x),
              Round(mouseTail[i].position.y), Round(smoke.width * mouseTail[i].size),
              Round(smoke.height * mouseTail[i].size)),
              TVector2.Create(smoke.width * mouseTail[i].size / 2,
